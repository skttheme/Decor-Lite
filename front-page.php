  ...
  <div class="slider-wrapper theme-default">
    <div id="slider" class="nivoSlider">
        ...
        while ( $query->have_posts() ) : $query->the_post();
          $i++;
          $decor_lite_slideno[] = $i;
          $decor_lite_slidetitle[] = get_the_title();
		  $decor_lite_slidedesc[] = get_the_excerpt();
          $decor_lite_slidelink[] = esc_url(get_permalink());
          ?>
        ...
        <?php
        $k = 0;
        foreach( $decor_lite_slideno as $decor_lite_sln ){ ?>
    <div id="slidecaption<?php echo esc_attr( $decor_lite_sln ); ?>" class="nivo-html-caption">
      <div class="slide_info">
        <h2><?php echo esc_html($decor_lite_slidetitle[$k] ); ?></h2>
        <?php if(!empty($decor_lite_slidedesc[$k])){?>
        <p><?php echo esc_html($decor_lite_slidedesc[$k] ); ?></p>
        <?php } ?>
        <div class="clear"></div>
        <a class="slide_more" href="<?php echo esc_url($decor_lite_slidelink[$k] ); ?>">
          <?php esc_html_e('Read More', 'decor-lite');?>
          </a>
      </div>
    </div>
 	<?php $k++;
        ...
        <?php
                    if ( have_posts() ) :
						...
						the_posts_pagination( array(
							'mid_size' => 2,
							'prev_text' => esc_html__( 'Back', 'decor-lite' ),
							'next_text' => esc_html__( 'Next', 'decor-lite' ),
						) );
                    else :
